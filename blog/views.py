...
def index(request):
    popular_posts = Post.objects.annotate(Count('likes')).order_by('-likes__count').prefetch_related('author')
    most_popular_posts = list(popular_posts)[:5]

    fresh_posts = Post.objects.order_by('published_at').prefetch_related('author')
    most_fresh_posts = list(fresh_posts)[-5:]

    popular_tags = Tag.objects.all().annotate(Count('posts')).order_by('-posts__count')
